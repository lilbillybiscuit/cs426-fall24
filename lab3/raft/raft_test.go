...
func TestReElection3A(t *testing.T) {
	...
	println("Disconnected the leader ", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	println("Reconnected the leader ", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect((leader2 + 1) % servers)
	println("Disconnected the leader2 ", (leader2+1)%servers)
	cfg.disconnect(leader2)
	println("Disconnected the leader2 ", leader2)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	println("Reconnected the leader2 ", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	println("Reconnected the leader2 ", leader2)
	cfg.checkOneLeader()
