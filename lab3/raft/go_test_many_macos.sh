...
logs=$(find . -maxdepth 1 -name 'test-*.log' -type f -print | wc -l)
success=$(grep -E '^PASS$' test-*.log | wc -l)
((failed = logs - success))

finish() {
    if ! wait "$1"; then
        if command -v osascript >/dev/null 2>&1 && ((failed == 0)); then
            osascript -e 'display notification "Tests started failing" with title "Test Status"'
        fi
        ((failed += 1))
    else
        ((success += 1))
